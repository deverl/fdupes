...
void walk_files_manual(
    ...
    for (auto& e : fs::directory_iterator(dir, ec)) {
        ...
        if (min_depth.has_value() && depth < *min_depth) {
            continue;
        }
        if (max_depth.has_value() && depth > *max_depth) {
            continue;
...
Stats compute_stats(const std::vector<Group>& groups) {
    ...
    for (const auto& g : groups) {
        ...
        if (!g.empty()) {
            ...
            if (!ec) {
                s.wasted_bytes += static_cast<int64_t>((g.size() - 1) * sz);
            }
        }
    }
    return s;
...
void json_escape(std::ostream& out, const std::string& s) {
    ...
    for (char c : s) {
        if (c == '"') {
            ...
            out << "\\u" << std::hex << std::setfill('0') << std::setw(4) << static_cast<unsigned>(c) << std::dec;
        } else {
            out << c;
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    [[maybe_unused]] bool    no_recurse = false;
    int                      jobs       = 1;
    bool                     stats      = false;
...
options:
  ...
  -f, --format FORMAT   Output format: text or json (default: text)
        ...
        if (arg == "-x" || arg == "--exclude") {
            if (i + 1 >= argc) {
                std::cerr << "Error: -x requires an argument\n";
                return 1;
            }
        ...
        if (arg == "--exclude-from") {
            if (i + 1 >= argc) {
                std::cerr << "Error: --exclude-from requires an argument\n";
                return 1;
            }
            exclude_from = argv[++i];
            continue;
        }
        if (arg == "-z" || arg == "--ignore-empty") {
            ignore_empty = true;
            continue;
        }
        if (arg == "--min-depth") {
            if (i + 1 >= argc) {
                std::cerr << "Error: --min-depth requires an argument\n";
                return 1;
            }
        ...
        if (arg == "--max-depth") {
            if (i + 1 >= argc) {
                std::cerr << "Error: --max-depth requires an argument\n";
                return 1;
            }
        ...
        if (arg == "--min-size") {
            if (i + 1 >= argc) {
                std::cerr << "Error: --min-size requires an argument\n";
                return 1;
            }
        ...
        if (arg == "--max-size") {
            if (i + 1 >= argc) {
                std::cerr << "Error: --max-size requires an argument\n";
                return 1;
            }
            auto v = parse_size(argv[++i]);
            if (!v) {
        ...
        if (arg == "-j" || arg == "--jobs") {
            if (i + 1 >= argc) {
                std::cerr << "Error: -j requires an argument\n";
                return 1;
            }
